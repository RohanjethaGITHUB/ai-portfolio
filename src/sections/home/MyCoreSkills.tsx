...
function setGlobalFlip(on: boolean) {
  if (typeof document === "undefined") return;
  if (on) {
    document.documentElement.setAttribute("data-card-flip", "1");
  } else {
    document.documentElement.removeAttribute("data-card-flip");
  }
}

export function MyCoreSkills() {
  ...
  return (
    <section
      className="section cardSectionBg"
      id="what-i-can-do"
      onMouseEnter={() => setGlobalFlip(true)}
      onMouseLeave={() => setGlobalFlip(false)}
      onFocus={() => setGlobalFlip(true)}
      onBlur={() => setGlobalFlip(false)}
    >
