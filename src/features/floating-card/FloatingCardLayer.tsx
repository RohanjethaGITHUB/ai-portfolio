    ...
    function applyTransform(immediate: boolean, dt: number) {
      if (immediate) {
        ...
        if (flipDiff < SNAP_FLIP) {
          ...
          [current.flip, vel.flip] = springStep(
            current.flip,
            vel.flip,
            target.flip,
            FLIP_SPRING,
        ...
        if (Math.abs(target.x - current.x) < SNAP_POS) {
          current.x = target.x;
          vel.x = 0;
        }
        if (Math.abs(target.y - current.y) < SNAP_POS) {
          current.y = target.y;
          vel.y = 0;
        }
        if (Math.abs(target.r - current.r) < SNAP_ROT) {
          current.r = target.r;
          vel.r = 0;
        }
        if (Math.abs(target.s - current.s) < SNAP_SCALE) {
          current.s = target.s;
          vel.s = 0;
        }
      }
      ...
      innerEl.style.transform = `rotateY(${current.flip}deg)`;
    }
